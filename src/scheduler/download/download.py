...
def download_dataset(s3_bucket: str, remote_path: str, output_path: str):
    remote_path = f"{remote_path}/demand_orders.csv"
    output_local_path = f"{output_path}/demand_orders_status.csv"

    logger.info(f"remote_path: {remote_path}")
    logger.info(f"output_local_path: {output_local_path}")

    os.makedirs(output_path, exist_ok=True)
    download_file(bucket_name=s3_bucket, remote_path=remote_path, local_path=output_local_path,)
